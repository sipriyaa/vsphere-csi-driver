	...
	*/
	...
	ginkgo.It("Taking snapshot of statefulset pvcs and attaching "+
		...
		volumeSnapshot2, snapshotContent2, snapshotCreated2,
			...
			volumeSnapshotClass, pvclaim2, volHandle2, "1Gi", true)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
		...
		volumeSnapshot3, snapshotContent3, snapshotCreated3,
			...
			volumeSnapshotClass, pvclaim3, volHandle3, "1Gi", true)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		defer func() {
		...
		err = verifyPVnodeAffinityAndPODnodedetailsForDeploymentSetsLevel5(ctx, client, deployment1,
			namespace, allowedTopologies, false)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		err = verifyPVnodeAffinityAndPODnodedetailsForDeploymentSetsLevel5(ctx, client, deployment2,
			namespace, allowedTopologies, false)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		err = verifyPVnodeAffinityAndPODnodedetailsForDeploymentSetsLevel5(ctx, client, deployment3,
			namespace, allowedTopologies, false)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
