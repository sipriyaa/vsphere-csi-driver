...
func createStaticVolumeOnSvc(ctx context.Context, client clientset.Interface, namespace string, datastoreUrl string,
	...
	for _, dc := range datacenters {
		...
		defaultDatastore, err = getDatastoreByURL(ctx, datastoreUrl, defaultDatacenter)
		if err != nil {
			return "", nil, nil, nil, fmt.Errorf("failed to get datastore from URL '%s' in datacenter '%s': %v", datastoreUrl, dc, err)
		}
	}
	...
	if err != nil {
		return "", nil, nil, nil, fmt.Errorf("failed to create FCD with profile ID '%s': %v", profileID, err)
	}
	...
	if os.Getenv(envPandoraSyncWaitTime) != "" {
		...
		if err != nil {
			return "", nil, nil, nil, fmt.Errorf("invalid Pandora sync wait time '%s': %v", os.Getenv(envPandoraSyncWaitTime), err)
		}
	} else {
	...
	if err != nil {
		return "", nil, nil, nil, fmt.Errorf("failed to create CNS register volume for FCD '%s': %v", fcdID, err)
	}
	...
	pvc, err := client.CoreV1().PersistentVolumeClaims(namespace).Get(ctx, pvcName, metav1.GetOptions{})
	if err != nil {
		return "", nil, nil, nil, fmt.Errorf("failed to get PVC '%s' from namespace '%s': %v", pvcName, namespace, err)
	}

	pv := getPvFromClaim(client, namespace, pvcName)
	if pv == nil {
		return "", nil, nil, nil, fmt.Errorf("failed to retrieve PV for PVC '%s' in namespace '%s'", pvcName, namespace)
	}
