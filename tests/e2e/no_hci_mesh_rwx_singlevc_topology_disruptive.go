...
var _ = ginkgo.Describe("[rwx-nohci-singlevc-disruptive] RWX-Topology-NoHciMesh-SingleVc-Disruptive", func() {
	...
	ginkgo.It("When full site Az2 is down", ginkgo.Label(p1, file, vanilla, level5, level2,
		...
		for i := 0; i < len(pvclaims); i++ {
			if i == 0 {
				deploymentList, podsNodeSelectorterm, err = createVerifyAndScaleDeploymentPods(ctx, client, namespace,
					int32(replica), false, labelsMap, pvclaims[i], nodeSelectorTerms, execRWXCommandPod,
		...
		for i := 0; i < 3; i++ {
			_, _, _ = createVerifyAndScaleDeploymentPods(ctx, client, namespace, int32(replica),
				true, labelsMap, pvclaims[i], nodeSelectorTerms, execRWXCommandPod, nginxImage, true, depl[i], 0)
		}
	...
	*/
	...
	ginkgo.It("When Az2 and Az3 sites are partially down", ginkgo.Label(p1, file, vanilla,
		...
		for i := 0; i < len(pvclaims); i++ {
			deploymentList, pods, err := createVerifyAndScaleDeploymentPods(ctx, client, namespace, int32(replica), false, labelsMap,
				pvclaims[i], nil, execRWXCommandPod, nginxImage, false, nil, createDepItr)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
			depl = append(depl, deploymentList...)
	...
	/*
		...
		Steps:
		1. Create SC with Immediate Binding mode and set SC with all levels of allowed topology,
		pass storage policy specific to cluster3
		2. Create multiple rwx pvcs around 10
		3. Wait for all PVCs to reach Bound state
		4. Create deployment Pods for each rwx created pvc with replica count 2
		5. Wait for pods to reach running ready state
		6. Verify that all the Pods are spread across all AZs
		7. Create statefulset with replica count 3.
		8. Wait for stateful pvcs to reach Bound state and Pods to reach running state
		9. PSOD all ESXI hosts in Az2 and Az3
		10. Create new set of rwx pvcs when PSOD is performed on Az2 and Az3
		11. Perform scaleup operation on statefulset and deployment. Increase the replica count of each to count 5.
		12. Attach standalone pods to newly created rwx pvcs.
		13. Verify all PVCs and Pods reach to Bound and running state.
		14. ESXI hosts were PSOD was performed should come back to healthy running state.
		15. Wait for testbed to be back to normal state.
		16. Verify that all the workload Pods are in up and running state.
		17. Perform scaling operation on deployment pods and statefuset pods once both sites are recovered from psod.
		18. Perform cleanup. Delete Pods, PVCs and PVs.
	*/
	...
	ginkgo.It("Perform psod on all esxi hosts of Az2 and Az3", ginkgo.Label(p1, file, vanilla,
		...
		ginkgo.By("Fetch list of hosts in cluster2 and cluster3")
		var hostList []string
